...
typedef struct Lexer {
    ...
    String_ID keyword_ids[MAX_KEYWORD_TOKEN_COUNT];
    
    Token *active_token;
} Lexer;

Lexer *create_lexer(struct Compiler_Ctx *ctx, struct In_Stream *stream, File_ID file);
...
uptr fmt_tok(struct Compiler_Ctx *ctx, char *buf, uptr buf_sz, Token *token);